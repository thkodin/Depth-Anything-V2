...
def main():
    ...
    parser.add_argument(
        "--encoder", default="vitl", type=str, choices=["vits", "vitb", "vitl", "vitg"], help="Model encoder to use."
    )
    parser.add_argument(
        "--load-from", default="", type=str, required=True, help="Path to the pre-trained model weights."
    )
    parser.add_argument("--max-depth", default=20, type=float, help="Maximum depth value for the depth map.")
    parser.add_argument(
        "--img-path", type=str, required=True, help="Path to the input image or directory containing images."
    )
    parser.add_argument(
        "--outdir", type=str, default="./vis_pointcloud", help="Directory to save the output point clouds."
    )
    ...
    for k, filename in enumerate(filenames):
        ...
        o3d.io.write_point_cloud(
            os.path.join(args.outdir, os.path.splitext(os.path.basename(filename))[0] + ".ply"), pcd
        )
